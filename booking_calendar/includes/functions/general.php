...
function make_hidden_fields($fields = array())
{
  ...
  foreach ($fields as $field) {
	if (@$_REQUEST[$field] != "") { $hidden_fields .= '<input type="hidden" name="' . $field . '" value="' . htmlspecialchars($_REQUEST[$field]) . '" />' ."\n"; }
  }
...
function make_hidden_fields_workstring($fields = array())
{
  ...
  foreach ($fields as $field) {
	if (@$_REQUEST[$field] != "") { $hidden_fields_workstring .= '&' . $field . '=' . urlencode($_REQUEST[$field]); }
  }
